...
function index() {
  ...
  const posts = data.investments?.filter((investment) => investment.id == pId);
  return (
    <div>
      {posts.map((post) => (
        <div key={post.id} className="bg-gray px-lp py-4xl sm:px-none sm:py-pp tablet:px-sp xl:px-res">
          <div className="flex flex-col bg-white py-xl px-sp">
            ...
            <div className="flex justify-between items-center  mt-xl 	">
              ...
              <Link
                href={`http://localhost:3000/education/investment/` + post.ID}
              >
              ...
              <Link
                href={`http://localhost:3000/education/investment/` + post.ID}
              >
                <button className="mBtn-cart hidden sm:flex">
                  <span className="ml-mm sm:text-med sm:ml-lm">
                    {post.text}
                  </span>
                  <Chevron />
